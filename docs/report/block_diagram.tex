...
\begin{circuitikz}
  ...
  % Highlight
  \draw[red, ultra thick, rounded corners] (seq.west |- redpitaya.south west) ++(-0.1,-0.6) rectangle ([shift={(0.3,0.6)}]redpitaya.north east);
  \draw[red, ultra thick, rounded corners] (rptx) ++(0.5,-0.9) rectangle ([shift={(-0.5,1.3)}]tx);
  \draw[red, ultra thick, rounded corners] (circ.w |- circ.s) ++(-1.4,-0.3) rectangle ([shift={(0.3,0.3)}]circ.e |- circ.n);
  \draw[red, ultra thick, rounded corners] (probe.south west) ++(-0.3,-0.8) rectangle ([shift={(0.3,0.3)}]probe.north east);
  \draw[red, ultra thick, rounded corners] (rprx) ++(0.5,-1.3) rectangle ([shift={(-0.5,0.9)}]rx);
\end{circuitikz}
\end{document}